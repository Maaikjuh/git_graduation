...
class BasisVectorsBayer(object):
    ...
    def project_gradient(u, V, quadrature_degree=None):
        ...
        # solve
        v1 = Function(V)
        v1.vector().set_local(grad_eps.get_local() / Md.get_local())
    ...
    def _create_vectors_lv(self, V):
       ...
       """
        ...
        grad_psi = grad_psi_.vector().get_local().reshape(-1, 3)
        grad_phi = grad_phi_.vector().get_local().reshape(-1, 3)
    ...
    def _create_wall_bounded_coords(self, V, phi_rv, phi_lv, phi_epi, grad_phi_rv, grad_phi_lv, psi_func):
        ...
        u = psi_to_u(psi_func.vector().get_local())
        self._u = u
        ...
        if self.parameters['transmural_coordinate_mode'] == 2:  # New method.
            ...
            v_septum = v_septum_func.vector().get_local()
    ...
    def _create_vectors_biv(self, V):
        ...
        if self.parameters['mirror']:
            ...
            grad_phi_epi = grad_phi_epi_.vector().get_local().reshape(-1, 3)
            grad_phi_lv = grad_phi_lv_.vector().get_local().reshape(-1, 3)
            grad_psi = grad_psi_.vector().get_local().reshape(-1, 3)

        # Convert functions to arrays (values at dofs).
        phi_epi = phi_epi_.vector().get_local()
        phi_lv = phi_lv_.vector().get_local()
        ...
        if self.parameters['interp_mode'] == 2:
            ...
            grad_phi_endo = self.project_gradient(phi_endo, V).vector().get_local().reshape(-1, 3)
            ...
            grad_phi_endo = self.project_gradient(phi_endo, V, quadrature_degree=7).vector().get_local().reshape(-1, 3)
        else:
            grad_phi_endo = None
    ...
    def mirror(self, X, grad_phi_epi_, grad_phi_lv_, grad_psi_):
        ...
        for idx in range(3):
            ...
            array_local = func_local.vector().get_local().reshape(-1, 3)
...
class GeoFunc(object):
    ...
    def interpolate_v_septum(geometry, v_septum_func):
        ...
        v_septum_i = true_dist_sep(v_septum_func.vector().get_local())
